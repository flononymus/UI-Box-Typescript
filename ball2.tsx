    ...
    useEffect(() => {
        ...
        const render = () => {
            if (!isDragging) {
                ...
                // Stop ball inside the hoop
                if (!isColliding && 
                    ballX > hoop.centerX - hoop.width / 2 && 
                    ballX < hoop.centerX + hoop.width / 2 &&
                    ballY > hoop.centerY && 
                    ballY < hoop.centerY + hoop.height
                ) {
                    vx = 0;
                    vy = 0;
                    ballX = Math.min(Math.max(ballX, hoop.centerX - hoop.width / 2 + radius), hoop.centerX + hoop.width / 2 - radius);
                    ballY = Math.min(Math.max(ballY, hoop.centerY + radius), hoop.centerY + hoop.height - radius);
                }
                ...
                if (ballY + radius > wh || ballY - radius < 0 + navbar.offsetHeight) {
                    ...
                    if (ballY - radius < 0 + navbar.offsetHeight) ballY = navbar.offsetHeight + radius;
                }
